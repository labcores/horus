        ...
        while(continua == 1):
                try:
                        ...
                        if(max_id == '0'):
                                ...
                                URL = "https://api.twitter.com/1.1/search/tweets.json?geocode="+geo+"&since="+since+"&until="+until+"&count=100"+"&lang=pt"
                        else:

                                URL = "https://api.twitter.com/1.1/search/tweets.json?geocode="+geo+"&since="+since+"&until="+until+"&count=100"+"&lang=pt"+"&max_id="+str(max_id)
                        max_id_ant = max_id
                        response, data = client.request(URL, "GET")
                        debateBandPresidencia20180809 = json.loads(data)
                        for tweet in debateBandPresidencia20180809['statuses']:
                                                        
                                db.debateBandPresidencia20180809.update({'id': tweet['id']},tweet, upsert=True)
